  ...
  <div v-if="app!=null"
    ...
    <div class="content pretty-scroll">
      ...
      <div v-if="state==='install'" class="text">
        <span v-if="app.type === 'theme'">{{ LANG.manager.float_installer_message_template }}</span>
      ...
      <div v-else-if="state==='error'" class="text">{{ LANG.manager.not_installed }}</div>
    ...
    <div class="actions">
      ...
      <div v-else-if="state==='complete'">
        <div @click="goToAppManager(app.type === 'theme' ? 'app-templates' : 'app-details')" class="btn-pin">
          <span v-if="app.type === 'theme'"> {{ LANG.manager.go_to_templates }}</span>
...
export default {
  ...
  methods: {
    ...
    goToAppManager(route) {
      this.close();
      let app = !!this.app.app ? this.app.app : this.app.package_name;
      this.$router.push({name: route, params: {package_name: app}});
    },
    update(state) {
      switch (state) {
        case 'market':
    ...
    installThemeManual() {
      this._loading = true;
      this.state = 'installing';
      this.$http.get(this.URL.API + 'template/installPackage/' + this.app.filename).then((json) => {
        this._loading = false;
        this.sendNotify(json.data, this.LANG.manager.install);
      });
    },
