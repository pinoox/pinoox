...
export function resolveInstallerRoot() {
    if (hasBoot() && getUrl().BASE) {
        const fromConfig = normalizeInstallerBasePath(getUrl().BASE);
...
export async function checkBootstrapLoad() {
    return {
        ok: hasBoot(),
        status: hasBoot() ? 200 : null,
        inline: true,
        useQueryRoute: shouldUseQueryRoute(),
    };
}

/** @deprecated use checkBootstrapLoad */
export const checkPinooxJsLoad = checkBootstrapLoad;
...
export function isBootstrapReady(ping, results) {
    ...
    if (!hasBoot() || shouldCheckStep3({rewrite: results.rewrite, htaccess: results.htaccess}, ping)) {
...
export async function runBootstrapCheckPinooxJs() {
    if (hasBoot() && !shouldUseQueryRoute()) {
        return {state: 'pass'};
    }

    const useQueryFirst = shouldUseQueryRoute();
    const primary = await checkBootstrapLoad();
