...
it('resolves configurable project paths from env values', function () {
    ...
    expect(SystemConfig::path('pinker'))->toBe($basePath . '/tests/Fixtures/system_config/custom_pinker')
        ...
        ->and(SystemConfig::path('wizard_tmp'))->toBe($basePath . '/tests/Fixtures/system_config/custom_pinker/wizard_tmp');
});

it('uses top-level patch paths outside the database folders', function () {
    $basePath = str_replace('\\', '/', testProjectRoot());

    expect(SystemConfig::path('system_patches'))->toBe($basePath . '/pincore/patches')
        ->and(SystemConfig::path('platform_patches'))->toBe($basePath . '/pincore/patches')
        ->and(SystemConfig::platformPath('migrations'))->toBe($basePath . '/pincore/database/migrations')
        ->and(SystemConfig::platformPath('patches'))->toBe($basePath . '/pincore/patches')
...
{
    return str_replace('\\', '/', testProjectRoot() . '/tests/Fixtures/system_config');
}
