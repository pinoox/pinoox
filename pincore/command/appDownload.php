	...
	*/
	...
	private function login(){
        ...
        if ( is_null($token_key) ) {
            ...
            if ($data3['status']) {
                Config::set('connect.token_key', $data3['result']['token']);
                Config::save('connect');
    ...
    private function downloadRequest($package_name, $token)
    {
        ...
        $params =  [
            'token' => $token,
            'remote_url' => 'http://pinoox-cli/',
            'user_agent' => HelperHeader::getUserAgent() . ';Pinoox/' . Config::get('~pinoox')['version_name'] . ' Manager;pinoox-cli',
        ];

        $res = HttpRequest::init('https://www.pinoox.com/api/manager/v1/market/downloadRequest/' . $package_name, HttpRequest::POST ,false)->params($params)->send();
        if (!empty($res)) {
            $response = json_decode($res, true);
            if (!$response['status']) {
                $this->error('Can not download it now!');
            } else {
                $path = path("downloads>apps>" . $package_name . ".pin" , 'com_pinoox_manager');
