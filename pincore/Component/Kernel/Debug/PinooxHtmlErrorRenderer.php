...
use Pinoox\Component\Kernel\Debug\Support\NetworkPreviewDetector;
 ...
 */
class PinooxHtmlErrorRenderer implements ErrorRendererInterface
{
    ...
    public function getStylesheet(): string
    {
        ...
        return $this->include('assets/css/exception.css') . $this->include('assets/css/pinoox-exception.css');
    }
    ...
    private function renderException(FlattenException $exception, string $debugTemplate = 'views/exception_full.html.php'): string
    {
        ...
        $pinoox = ExceptionContext::collect($exception);
        $networkPreview = NetworkPreviewDetector::prefersEmbeddedDebugPage();
        ...
        return $this->include($debugTemplate, [
            ...
            'pinoox' => $pinoox,
            'hints' => ExceptionHintResolver::resolve($exception, $pinoox),
    ...
    private function resourceDir(): string
    {
        if (defined('PINOOX_CORE_PATH')) {
            return rtrim(PINOOX_CORE_PATH, '/\\') . '/resource/debug/';
        }

        return dirname(__DIR__, 3) . '/resource/debug/';
    }
    ...
    private function include(string $name, array $context = []): string
    {
        ...
        $path = $this->resourceDir() . str_replace('\\', '/', $name);
        include is_file($path) ? $path : $name;
