...
trait SelectsTestPackage
{
    protected function resolvePackage(InputInterface $input, OutputInterface $output, SymfonyStyle $io): string
    {
        ...
        if (empty($package) && $input->hasOption('app')) {
            $package = $this->normalizePackageInput((string)$input->getOption('app'));
        }
        ...
        $io->section('Available test packages');
        $io->table(['#', 'Package', 'Name', 'Tests'], $this->packageRows($packages));
    ...
    protected function packages(): array
    {
        ...
        foreach (AppEngine::all() as $package => $manager) {
            $packages[$package] = $manager->config()->get('name') ?: $package;
        }
    ...
    protected function packageRows(array $packages): array
    {
        ...
        foreach ($packages as $package => $name) {
            $rows[] = [count($rows), $package, $name, is_dir($this->testPath($package)) ? 'yes' : 'no'];
        }
